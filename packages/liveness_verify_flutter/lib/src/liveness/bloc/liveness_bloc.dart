...
class LivenessBloc extends Bloc<LivenessEvent, LivenessState> {
  ...
  Future<void> _startFrameStream(CameraController camera, LivenessStream stream, int generation) async {
    ...
    await camera.startImageStream((image) {
      if (sending || isClosed || generation != _generation || !identical(_stream, stream)) {
        return;
      }
      final blinkInProgress = status == LivenessStatus.blink || status == LivenessStatus.reopen;
      // The backend requires at least 80 ms between accepted frames. A 200 ms
      // cadence keeps normal multi-frame confirmations responsive while still
      // avoiding unnecessary CPU/network use. Blink needs a denser sample so a
      // short eye closure is not missed.
      final interval = blinkInProgress ? 100 : 200;
